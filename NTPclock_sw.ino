// NTP clock with manual switch for summer/winter time
// inspired by https://www.hackster.io/garysat/arduino-cheap-minimal-parts-digital-clock-0d750a
...
/* D1 Mini
  DIN connects to GPIO13 -> D7
  CLK connects to GPIO15 -> D8
  CS connects to GPIO14 -> D5
*/

LedControl lc = LedControl(13, 15, 14, 2);  // ESP8266  

#define orainplus 12 // GPIO12 = D6

byte mm1, mm5, h, m, s;
byte ultM = 10, ultH = 10;
//byte hPin = 7, mPin = 6;
// ajustaH = true, ajustaM = true;
byte secunda = 0;
byte secunda0 = 0;
byte lumina = 0;
...
const long utcOffsetInSeconds = 7200;  // 2x3600 = 2 hour + GMT (GMT+2)
...
void setup() {
  ...
  lc.setIntensity(0, 0);     // Set the brightness to custom value
  lc.clearDisplay(0);         // and clear the display
  lc.shutdown(1, false);      // power-saving mode on startup
  lc.setIntensity(1, 0);     // Set the brightness to cusmom value
...
timeClient.update();
  ...
  if (digitalRead(orainplus) == LOW) h = h + 1;  // add summer time
